...
export function RepoValidator() {
    ...
    try {
      ...
      if (!res.ok) {
        const data = await res
          .json()
          .catch(() => ({ ok: false, reason: `Server error: ${res.status}` }));
        setResult(data);
        return;
      }
  ...
  async function generateDocs() {
    if (!result || !result.ok || !url) return;
    setDocsLoading(true);
    setDocsError("");
    setDocs(null);
    try {
      const res = await fetch("/api/generate-docs", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ url, branch: selectedBranch || result.branch }),
      });
      if (!res.ok) {
        const data = await res
          .json()
          .catch(() => ({ ok: false, reason: `Server error: ${res.status}` }));
        setDocsError(data.reason || "Failed to generate documentation");
        return;
      }
      const data = await res.json();
      if (data.ok && data.docs) {
        // Navigate to documentation page with data
        navigate("/documentation", { state: { docs: data.docs } });
      } else {
        setDocsError(data.reason || "Failed to generate documentation");
      }
    } catch (err) {
      ...
      <div className="w-full">
        ...
        <div className="text-center mb-8 sm:mb-12 animate-fade-in">
          ...
          <p className="text-sm sm:text-base md:text-lg text-gray-400 max-w-2xl mx-auto px-4">
            Verify repository accessibility and code presence before generating
            SDLC documentation
          </p>
        </div>
        ...
        <div className="relative mb-6 sm:mb-8">
          ...
          <div className="relative rounded-2xl border border-white/10 bg-black/60 backdrop-blur-sm p-4 sm:p-6 md:p-8 shadow-2xl">
            ...
            {result && !loading && (
              <div
                className={`mt-4 flex items-center gap-2 text-sm animate-slide-up ${
          ...
          <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center animate-fade-in">
            <div className="text-center">
              ...
              <p className="text-gray-400 text-sm animate-pulse">
                Please wait while we validate your repository...
              </p>
            </div>
          </div>
        )}
        ...
        {result && !loading && (
          <div className="space-y-6 animate-slide-up">
            ...
            <div className="rounded-2xl border border-white/10 bg-gradient-to-br from-purple-900/20 via-black to-green-900/10 p-4 sm:p-6 backdrop-blur-sm">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
                ...
                <div className="group relative flex items-center gap-3 p-3 rounded-lg bg-black/40 border border-white/5 hover:border-blue-500/30 transition-all">
                  ...
                  <button
                    onClick={() =>
                      copyToClipboard(result.branch || "", "branch")
                    }
                    className="opacity-0 group-hover:opacity-100 transition-opacity p-1.5 hover:bg-white/10 rounded"
                    ...
                    {[...Array(5)].map((_, i) => (
                      <div
                        ...
                        className={`w-2 h-2 rounded-full ${
                          i < result.codeMetrics!.qualityScore
                            ? "bg-green-400"
                            : "bg-gray-700"
                        }`}
                      />
                  ...
                  <div className="flex flex-col sm:flex-row gap-3">
                    <div className="flex-1 p-3 rounded-lg bg-blue-500/10 border border-blue-500/30">
                      ...
                      <p className="font-semibold text-sm text-blue-300">
                        {result.codeMetrics.projectType}
                      </p>
                    </div>
                    <div className="flex-1 p-3 rounded-lg bg-green-500/10 border border-green-500/30">
                      ...
                      <p className="font-semibold text-sm text-green-300">
                        {result.codeMetrics.fileCount} files
                      </p>
                    </div>
                  </div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {result.codeMetrics.languages
                          ...
                          .map((lang, idx) => (
                            ...
                            >
                              {lang}
                              {result.codeMetrics?.languagePercentages?.[
                                lang
                              ] && (
              ...
              <div className="space-y-3">
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-400 mt-1">
                      {result.hasCode
                        ? "Code files detected in repository"
                        : "No code files found (docs only)"}
                    </p>
                  </div>
                </div>
              </div>
            ...
            {/* Next Steps & SDLC Docs Generation */}
            {result.ok && (
              <div className="rounded-2xl border border-green-500/30 bg-gradient-to-br from-green-900/20 to-black p-4 sm:p-6 backdrop-blur-sm animate-slide-up">
                <div className="flex flex-col sm:flex-row items-start gap-3">
                  ...
                  <div className="flex-1">
                    <h3 className="text-base sm:text-lg font-semibold mb-2 text-green-400">
                      Ready for Analysis!
                    </h3>
                    <p className="text-gray-300 text-sm mb-4">
                      This repository has passed all validation checks and is ready for architecture diagram generation and SDLC documentation.
                    </p>
                    <div className="flex flex-col sm:flex-row gap-3">
                      <button 
                      ...
                      <button 
                        className="flex items-center justify-center gap-2 w-full sm:w-auto rounded-lg px-6 py-2.5 font-medium bg-gradient-to-r from-green-600 to-green-500 hover:from-green-500 hover:to-green-400 transition-all transform hover:scale-105 shadow-lg shadow-green-500/25 text-sm"
                    ...
                    {docsError && (
                      <div className="mt-3 text-red-400 text-sm animate-slide-up">
                        {docsError}
                      </div>
                    )}
                  </div>
        ...
        {!result && !loading && (
          <div className="text-center py-16 animate-fade-in">
            ...
            <p className="text-gray-500">
              Enter a GitHub repository URL above to begin validation
            </p>
          </div>
        )}
      </div>
