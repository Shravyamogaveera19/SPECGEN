...
const CODE_DIRS = new Set([
  'src', 'lib', 'app', 'server', 'backend', 'api', 'services',
  'components', 'pages', 'views', 'controllers', 'models',
  'routes', 'handlers', 'middleware', 'utils', 'helpers',
  'core', 'common', 'shared', 'modules', 'packages',
]);
...
async function analyzeRepository(owner: string, repo: string, branch: string): Promise<CodeAnalysis> {
  ...
  try {
    ...
    if (treeResp.status !== 200) {
      result.reason = 'Unable to fetch repository tree';
      return result;
    }
    ...
    for (const file of files) {
      ...
      if (pathParts.some(part => TEST_DIRS.has(part))) {
...
function determineProjectType(languageCounts: Record<string, number>, configFiles: string[]): string {
  // Web frameworks
  ...
  // Python projects
  if (configFiles.includes('requirements.txt') || configFiles.includes('setup.py') || configFiles.includes('pyproject.toml')) {
